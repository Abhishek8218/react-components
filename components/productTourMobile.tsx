...
const ProductTourMobile: React.FC<ProductTourProps> = ({ steps, isOpen, onClose, onStepChange }) => {
  ...
  return (
    ...
    {/* Modal */}
    <div
      className="fixed z-50 pointer-events-auto bg-white px-2 py-1 shadow-lg rounded-lg  max-w-[60vw] max-h-[120px] "
      style={{
    ...
    >
      <div className={`absolute ${currentDirection === 'bottom' ? "bottom-[70px] left-[-100px] rotate-180" : currentDirection === 'top' ? "bottom-[-20px]  left-[-90px] -scale-x-100 -rotate-12 " : currentDirection === "right" ?  "bottom-[10px] left-[-95px] rotate-[180deg]" : ""}`} >
        <img src="curved-arrow.svg" alt="Profile" className="w-20 h-20 rounded-full mr-4" />
      </div>
      ...
      <div className="relative">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <h2 className="text-sm font-semibold">{steps[currentStep].title}</h2>
        ...
        <p className='text-xs'>{steps[currentStep].content}</p>
      </div>
    </div>
  </>
);
